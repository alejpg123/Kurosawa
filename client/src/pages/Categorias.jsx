    ...
    return (
        <div className="flex flex-col min-h-screen bg-gray-50">
            ...
            <div className="flex flex-col items-center justify-center flex-1 container mx-auto p-4">
                <h2 className="text-4xl font-bold mb-4 text-center text-gray-800">LISTADO DE CATEGORIAS</h2>
                <div className="bg-black w-full text-white py-4 flex items-center justify-center mb-4">
                <div className="flex items-center justify-center px-4">
                    <span>Como somos la tienda más completa de Argentina, te ayudamos para que elijas por categoría.</span>
                </div>
        </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
                    {uniqueCategories.map(item => (
                        <div
                            key={item.id}
                            className={`p-6 border rounded-lg shadow-md ${selectedCategory === item.category ? 'bg-blue-100 transform scale-105' : 'bg-white'} transition-all duration-300 transform hover:scale-105 cursor-pointer`}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 max-w-screen-lg mt-4">
                    {products.filter((prod) => (prod.category === selectedCategory)).map((product) => {
                        ...
                        return (
                            <div
                                key={product.id}
                                className={`w-80 bg-white shadow-lg rounded-lg overflow-hidden transition-transform duration-300 hover:scale-105 ${isClicked ? 'bg-indigo-200' : ''}`}
                            >
                                <div
